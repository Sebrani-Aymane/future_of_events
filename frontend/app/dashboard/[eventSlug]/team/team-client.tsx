...
type TeamMemberWithProfile = TeamMemberRow & { profile?: Database['public']['Tables']['profiles']['Row'] };
...
export default function TeamClient({
  ...
  const handleCreateTeam = async () => {
    ...
    try {
      ...
      const createdTeam = newTeam;

      // Add leader as team member
      const memberPayload: TeamMemberInsert = {
      ...
      const registrationUpdate: EventRegistrationUpdate = { team_id: createdTeam.id };
  ...
  const handleJoinTeam = async (teamId?: string) => {
    ...
    try {
      if (code) {
        ...
        if ((memberCount ?? 0) >= (teamData.max_members || event.max_team_size)) {
        ...
        const joinMemberPayload: TeamMemberInsert = {
          team_id: teamData.id,
          user_id: user.id,
          status: 'active',
        };
        ...
        const { error: memberError } = await supabase
          .from('team_members')
          .insert(joinMemberPayload);

        if (memberError) throw memberError;

        const registrationUpdate: EventRegistrationUpdate = { team_id: teamData.id };
        const { error: regError } = await supabase
          .from('event_registrations')
          .update(registrationUpdate)
          .eq('id', registration.id);
  ...
  const handleLeaveTeam = async () => {
    ...
    try {
      ...
      const registrationUpdate: EventRegistrationUpdate = { team_id: null };
      const { error: regError } = await supabase
        .from('event_registrations')
        .update(registrationUpdate)
        .eq('id', registration.id);
      ...
      if (isLeader && teamMembers.length > 1) {
        ...
        if (newLeader) {
          const teamUpdate: TeamUpdate = { leader_id: newLeader.user_id };
          await supabase
            .from('teams')
            .update(teamUpdate)
            .eq('id', team.id);
  ...
  const handleRemoveMember = async (memberId: string, memberName: string) => {
    ...
    try {
      ...
      const { data: memberReg, error: memberRegError } = await supabase
        .from('event_registrations')
        .select('id')
        .eq('event_id', event.id)
        .eq('user_id', memberId)
      ...
      if (memberRegError) throw memberRegError;
